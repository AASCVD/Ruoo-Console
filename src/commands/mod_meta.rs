// ============================================================
// RUOO-CONSOLE v8.0 — 模块自治命令注册表 (内核清理完毕)
...
const CORE: &[CmdDef] = &[
    ...
    ("exec_bg <命令> [shell]", "", "后台启动命令(无超时,实时输出推送终端,返回PID)", "核心命令"),
    ("exec_list", "", "列出所有后台进程(PID/状态/启动时间/命令)", "核心命令"),
    ("exec_kill <PID>", "", "终止后台进程(强制kill+清理资源)", "核心命令"),
    ("exec_send <PID> <文本>", "", "向后台进程发送stdin(支持交互式控制)", "核心命令"),
...
const FILE_OPS: &[CmdDef] = &[
    ...
    ("multirep / smi <文件> <JSON对> [大小写] [最大替换]", "", "流式多模式批量替换(一次遍历多个find→replace)", "文件操作"),
    ("searchexport <文件> <模式> <输出> [大小写] [上下文]", "", "搜索结果导出到文件(流式处理不爆内存)", "文件操作"),
    ("colex <文件> <列索引> [分隔符] [最大行]", "", "CSV/TSV流式列提取(comma/tab/pipe/semicolon)", "文件操作"),
    ("bindiff / binarydiff <文件1> <文件2> [最大差异]", "", "大文件二进制差异比较(64KB分块流式)", "文件操作"),
    ("chunkscan <文件> [块大小MB] [最大块数]", "", "分块扫描大文件(概览每块偏移/类型/可打印率)", "文件操作"),
    // v10.0 大文件流式操作 TUI快捷命令
    ("streamgrep <文件> <模式> [-i]", "", "流式搜索大文件(不加载到内存,BufReader逐行)", "文件操作"),
    ("streambin <文件> <HEX模式>", "", "流式二进制扫描(64KB分块搜索字节模式)", "文件操作"),
    ("ranged <文件> <偏移> [长度]", "", "按字节范围读取大文件(上限10MB,自动判断文本/二进制)", "文件操作"),
    ("sfr <文件> <查找> <替换> [-i]", "", "流式查找替换(边读边写不加载到内存,原子替换)", "文件操作"),
    ("bldidx <文件>", "", "构建行偏移索引(.idx,加速大文件随机行访问)", "文件操作"),
    ("seekline <文件> <起始行> [结束行]", "", "索引定位读取指定行(需先bldidx构建索引)", "文件操作"),
    ("qidx <文件>", "", "查询行索引状态(行数/文件大小/索引大小)", "文件操作"),
    ("sfinfo <文件>", "", "流式文件统计(行数/空行/最大行/非ASCII比例)", "文件操作"),
];
